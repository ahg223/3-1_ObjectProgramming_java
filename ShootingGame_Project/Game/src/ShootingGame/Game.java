...
public class Game {
	public static void main(String[] ar) {
		game_Frame AHG223 = new game_Frame();
	}
}
...
class game_Frame extends JFrame implements KeyListener, Runnable {
	...
	int x, y;
	int[] cx = { 0, 0, 0 }; // Background scroll speed control variable
	int bx = 0; // Background scroll control variable
	...
	int fire_Speed; 
	int enemy_speed; 
	int player_Status = 0;// 0 : normal, 1: launch missile, 2: crash with enemy
	int game_Score; 
	int player_Hitpoint;
	...
	game_Frame() {
		...
		setTitle("AHG223 슈팅 게임");
		setSize(f_width, f_height);
		Dimension screen = tk.getScreenSize();

		int f_xpos = (int) (screen.getWidth() / 4);
		int f_ypos = (int) (screen.getHeight() / 4);
	...
	public void init() {
		...
		for (int i = 0; i < Player_img.length; ++i) {
			Player_img[i] = new ImageIcon("player" + i + ".png").getImage();
		}
	...
	public void MissileProcess() {
		if (KeySpace) {
			...
			if ((cnt % fire_Speed) == 0) {
				...
				ms = new Missile(x + 80, y + 30, missile_Speed);
				Missile_List.add(ms);
			}
		}
	...
	public void EnemyProcess() {
		...
		if (cnt % 200 == 0) {
			en = new Enemy(f_width + 100, 100, enemy_speed + Ran.nextInt(5));
			Enemy_List.add(en);
			en = new Enemy(f_width + 100, 200, enemy_speed + Ran.nextInt(5));
			Enemy_List.add(en);
			en = new Enemy(f_width + 100, 300, enemy_speed + Ran.nextInt(5));
			Enemy_List.add(en);
			en = new Enemy(f_width + 100, 400, enemy_speed + Ran.nextInt(5));
			Enemy_List.add(en);
			en = new Enemy(f_width + 100, 500, enemy_speed + Ran.nextInt(5));
			Enemy_List.add(en);
	...
	public void Draw_Background() {
		...
		for (int i = 0; i < cx.length; ++i) {
			...
			buffg.drawImage(Cloud_img[i], 2000 - cx[i], i * 200, this);
	...
	public void Draw_StatusText() {
		...
		buffg.drawString("SCORE : " + game_Score, 1800, 80);
		buffg.drawString("HitPoint : " + player_Hitpoint, 1800, 100);
		buffg.drawString("Missile Count : " + Missile_List.size(), 1800, 120);
		buffg.drawString("Enemy Count : " + Enemy_List.size(), 1800, 140);
	}
